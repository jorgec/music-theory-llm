"""
Train Ultra-Large Music Theory Model (50M Parameters)

Configuration for maximum capacity:
- d_model: 512 (2x larger than v2)
- Layers: 8 (2x deeper than v2)
- Parameters: ~50M
...
def find_50m_config(vocab_size=101):
    """Find optimal configuration for ~50M parameters"""

    print("\nSearching for optimal 50M parameter configuration...\n")

    configs = []
    for d_model in [384, 448, 512, 576, 640]:
        for num_layers in [6, 7, 8, 9, 10]:
            for nhead in [8, 12, 16]:
                ...
                configs.append({
                    ...
                    'nhead': nhead,
                    'params': params,
                    'diff': abs(params - 50_000_000)
                })

    # Sort by closeness to 50M
    configs.sort(key=lambda x: x['diff'])

    # Show top 5 candidates
    print("Top 5 configurations closest to 50M parameters:")
...
def train_50m_model(
    ...
    save_dir: str = 'checkpoints_50m'
):
    """Train 50M parameter model"""

    print(f"\n{'='*80}")
    print(f"TRAINING 50M PARAMETER MUSIC THEORY MODEL")
    print(f"{'='*80}\n")

    # Find optimal config
    config = find_50m_config()
    ...
    # Create 50M model
    print(f"Creating 50M parameter model...")
    ...
    print(f"  Target achieved: {abs(actual_params - 50_000_000) / 50_000_000 * 100:.1f}% from 50M\n")
    ...
    print(f"\n{'='*80}")
    print(f"✓ 50M MODEL TRAINING COMPLETE!")
    print(f"{'='*80}")
    print(f"\nFinal Statistics:")
...
def main():
    ...
    parser.add_argument('--train-samples', type=int, default=4000,
                        help='Training samples (default: 4000)')
    parser.add_argument('--val-samples', type=int, default=800,
                        help='Validation samples (default: 800)')
    parser.add_argument('--lr', type=float, default=5e-5,
                        help='Learning rate (default: 5e-5)')
    parser.add_argument('--save-dir', type=str, default='checkpoints_50m',
                        help='Save directory')

    args = parser.parse_args()

    print(f"\n🎵 50M Parameter Music Theory Model Training 🎵")
    print(f"\nConfiguration:")
